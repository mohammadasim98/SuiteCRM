...
class SugarWidgetSubPanelSupplyChainButton extends SugarWidgetField
{
    ...
    public function displayList(&$layout_def)
    {
        ...
        if ($layout_def['EditView']) {
            ...
            write_to_log(array($relationship_name), 'Relationship name for Store and Supplychain');
            if ($relationship_name == 'wh_store_wh_supplychain') {
              $destroy_rel = 'index.php?entryPoint=Destroy&module='.$layout_def['module'].'&module_rec='.$layout_def['fields']['ID'];
              $destroy_rel .= '&host_record='.$_REQUEST['record'].'&rel_name='.$relationship_name;
            }
            ...
            return '<a href="'.$destroy_rel.'" onmouseover="' . $handler . '" onfocus="' . $handler .
                '" class="listViewTdToolsS1" id="' . $unique_id . '">' . $app_strings['LBL_SUPPLYCHAIN_BUTTON'] . '</a>';
        }
